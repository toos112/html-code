	...
	this._showNext = function() {
		if (this._displayPhase == 0) {
			...
			if (this._messageIndex >= this.messages.length) {
				...
				this.endTime = $.time() + Math.max(500, this.messages[this._messageIndex].msg.length * 100);
			...
			do { this._choiceIndex++;
				...
				for (var i = 0; i < thisRef.choices.length; i++)
					if (thisRef.choices[i].from == thisRef.ingame[thisRef._choiceIndex].id) return true;
				return false;
			}());
